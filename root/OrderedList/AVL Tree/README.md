...
> - ***节点类实现的AVL树***
>   - 首先这是一颗搜索二叉树，对于一个节点来说，它的左子树上的所有值都小于它，右子树上的都大于它（在有序表中我们认为这个值是key），且没有重复值
>   - 其次，AVL树多的条件是对于任何一个节点来说。左右子树之间的高度差不能大于1，为此有四种违规类型和对应的处理方式：
...
>   - AVL树本质上就是搜索二叉树打上了平衡性的补丁，实现有序表时用的还是搜索二叉树的方式，其增删改查时间复杂度为O(logN)
