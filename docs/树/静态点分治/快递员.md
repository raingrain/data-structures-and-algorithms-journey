...
> - **Question**
>   - 一共有 `n` 个节点，给定 `n - 1` 条边，每条边给定边权，所有节点组成一棵树，对于点对 `(a, b)` ，假设你选择的中心点为 `x` ，那么点对的距离如下 `点对(a, b)的距离 = a到x的路径权值和 + b到x的路径权值和` ，一共有 `m` 个点对，你需要选择中心点 `x` ，使得点对距离的最大值尽量小，打印这个最小的点对距离最大值。
>   - **Tips**
>     - `1 <= n、m <= 10^5`
