...
> - **Question**
>   - 两棵树 `t1` 、 `t2` 都有 `n` 个节点，都以 `1` 号节点作为树头。
>   - 各自给定 `n - 1` 条边，以及每条边的边权，边权可能为负数。
>   - 在 `t1` 中，点 `x` 到树头的距离记为 `dis1(x)` ，在 `t2` 中，该距离记为 `dis2(x)` 。
>   - 点 `x` 和点 `y` 在 `t1` 中的 `LCA` 记为 `lca1(x, y)` ，在 `t2` 中的 `LCA` 记为 `lca2(x, y)` 。
>   - `点对 (x, y) 的指标 = dis1(x) + dis1(y) - dis1(lca1(x, y)) - dis2(lca2(x, y))` 。
>   - 点对 `(x, y)` ，要求 `x <= y` ，打印这个指标的最大值。
>   - **Tips**
>     - `1 <= n <= 366666`
