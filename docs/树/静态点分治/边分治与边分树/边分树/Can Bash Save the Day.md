...
## [G. Can Bash Save the Day?](https://codeforces.com/problemset/problem/757/G)

> - **Question**
>   - 树上有 `n` 个节点，给定 `n - 1` 条边，每条边有边权。
>   - 给定长度为 `n` 的数组 `arr` ，代表点编号组成的一个排列。
>   - 接下来有 `q` 条操作，每条操作是如下两种类型中的一种。
>   - 操作 `1 x y z` 打印 `arr[x ~ y]` 中每个节点到节点 `z` 的简单路径距离之和。
>   - 操作 `2 x` 交换 `arr[x]` 和 `arr[x + 1]` 的值，输入保证 `1 <= x < n` 。
>   - **Tips**
>     - `1 <= n、q <= 2 * 10^5`
