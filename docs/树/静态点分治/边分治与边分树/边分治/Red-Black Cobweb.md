...
## [D. Red-Black Cobweb](https://codeforces.com/problemset/problem/833/D)

> - **Question**
>   - 一共 `n` 个节点，给定 `n - 1` 条边，所有节点组成一棵树。
>   - 每条边有两种边权， `w` 为重量， `c` 为颜色，颜色只有红和黑。
>   - 考虑点对 `(x, y)` 的简单路径， `x` 和 `y` 必须不同，假设红边数量 `r` 、黑边数量 `b` 。
>   - 如果 `2 * min(r, b) >= max(r, b)` ，那么路径是合法的。
>   - `合法路径的收益 = 路径所有边的重量乘积` ， `(x, y)` 和 `(y, x)` 的路径只算一次收益。
>   - 打印所有合法路径的收益总乘积，结果对 `1000000007` 取余，不存在任何合法路径打印 `1` 。
>   - **Tips**
>     - `2 <= n <= 10^5`
>     - `1 <= w <= 10^9 + 6`
