...
> - **Question**
>   - 树上有 `n` 个点，每个点的初始点权是 `0` ，给定 `n - 1` 条边，每条边有边权。如果点 `x` 是指挥点，它指挥点 `y` 的花费等于 `x` 到 `y` 的简单路径权值和乘以 `y` 的点权。树上存在某个最佳的指挥点，指挥所有点的总花费最小，叫做最小指挥总花费。一共 `m` 条操作，格式 `x v` 先把 `x` 的点权增加 `v` ，然后打印此时的最小指挥总花费。注意参数 `v` 有可能是负数，但题目保证任何时候，点权不会出现负数。
>   - **Tips**
>     - `1 <= n, m <= 10^5`
>     - `1 <= 边权 <= 1000`
>     - `-1000 <= v <= +1000`
