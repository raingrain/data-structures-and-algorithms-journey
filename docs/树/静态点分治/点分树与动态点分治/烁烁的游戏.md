...
## [P10603 BZOJ4372 烁烁的游戏](https://www.luogu.com.cn/problem/P10603)

> - **Question**
>   - 树上有 `n` 个点，每个点的初始点权是 `0` ，给定 `n - 1` 条边，边权都是 `1` ，接下来有 `m` 条操作，每条操作是如下两种类型中的一种。
>     - 操作 `M x k v` ：点 `x` 出发，距离 `<= k` 的所有点，点权都加上 `v` 。
>     - 操作 `Q x` ：打印点 `x` 的点权。
>   - **Tips**
>     - `1 <= n、m <= 10^5`
>     - `-10000 <= v <= +10000`
