...
## [P10604 BZOJ4317 Atm 的树](https://www.luogu.com.cn/problem/P10604)

> - **Question**
>   - 树上有 `n` 个点，给定 `n - 1` 条边，每条边有边权。现在关心，从节点 `x` 出发到达其他点的距离中，第 `k` 小的距离。注意，节点 `x` 到自己的距离 `0` ，不参与距离评比。给定正数 `k` ，打印每个节点的第 `k` 小距离，一共 `n` 条打印。
>   - **Tips**
>     - `1 <= n <= 15000`
>     - `1 <= k <= 5000`
>     - `1 <= 边权 <= 10`
