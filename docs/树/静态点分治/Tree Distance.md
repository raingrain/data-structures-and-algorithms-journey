...
> - **Question**
>   - 一共有 `n` 个节点，给定 `n - 1` 条边，每条边给定边权，所有节点组成一棵树。节点 `i` 到节点 `j` 的简单路径，边权的累加和，定义为 `dist(i, j)` 。编号区间 `[x, y]` ，考虑所有点对 `(a, b)` ，要求 `x <= a < b <= y` 。如果 `dist(a, b)` 是所有情况中最小的，则称 `(a, b)` 为区间 `[x, y]` 的支配点对 。也可以说，区间 `[x, y]` 的支配点对距离为 `dist(a, b)` 。特别的，如果 `x == y` ，那么不存在满足要求的点对，此时支配点对距离为 `-1` 。一共有 `m` 次查询，格式 `x y` : 保证 `x <= y` ，打印 `[x, y]` 的支配点对距离。
>   - **Tips**
>     - `1 <= n <= 2 * 10^5`
>     - `1 <= m <= 10^6`
>     - `1 <= 边权 <= 10^9`
