...
> - **Question**
>   - 一共有 `n` 个点，给定 `n - 1` 条边，所有节点组成一棵树，每个点有点权，是一个大写字母，只考虑大写字母 `A` 到 `Z` ，给定一个长度为 `m` 的字符串 `s` ，也只由大写字母 `A` 到 `Z` 组成，考虑点对 `(u, v)` 的简单路径，把沿途节点的字母拼接起来，如果拼接字符串恰好是 `s` 重复正数次，那么该点对合法，打印合法点对的数量，注意 `(u, v)` 和 `(v, u)` 是不同的点对。
>   - **Tips**
>     - `1 <= m <= n <= 10^5`
