...
> - **Question**
>   - 一共有 `n` 个节点，给定 `n - 1` 条无向边，所有节点组成一棵树，如果在节点 `a` 和节点 `b` 之间建立新通道，那么代价是两个节点在树上的距离，一共有 `q` 条查询，每条查询格式如下：查询 `k a1 a2 ... ak` ：给出了 `k` 个不同的节点，任意两个节点之间都会建立新通道，打印新通道的代价和、新通道中代价最小的值、新通道中代价最大的值。
>   - **Tips**
>     - `1 <= n <= 10^6`
>     - `1 <= q <= 5 * 10^4`
>     - `1 <= 所有查询给出的点的总数 <= 2*n`
