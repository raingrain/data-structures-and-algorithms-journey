...
>     - `v 3 x` ：基于 `v` 版本的树，查询 `x` 的排名，生成新版本的树状况等于 `v` 版本状况。
>     - `v 4 x` ：基于 `v` 版本的树，查询数据中排名为 `x` 的数，生成新版本的树状况等于 `v` 版本状况。
>     - `v 5 x` ：基于 `v` 版本的树，查询 `x` 的前驱，生成新版本的树状况等于 `v` 版本状况。
>     - `v 6 x` ：基于 `v` 版本的树，查询 `x` 的后继，生成新版本的树状况等于 `v` 版本状况。
...
// 可持久化的重点是理解底层节点会有复用的情况
// i号版本树的头，记录在head[i]中，每个版本的头都记录
