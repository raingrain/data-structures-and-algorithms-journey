...
> - **Question**
>   - 图里有 `n` 个点， `m` 条无向边，点有点权，边有边权，图里可能有若干个连通的部分，一共有 `q` 条查询，查询格式如下，查询 `u x k` ：从点 `u` 开始，只能走过权值 `<=x` 的边，所有能到达的点中，打印第 `k` 大点权，如果不存在打印 `-1` 。
>   - **Tips**
>     - `1 <= n <= 10^5`
>     - `0 <= m、q <= 5 * 10^5`
>     - `1 <= 点权、边权 <= 10^9`
...
> - **Question: 离散化 + Kruskal重构树 + 叶节点dfn序依次建立主席树 + 查询子树中第k大的点权**
