...
## [AT_agc002_d [AGC002D] Stamp Rally](https://www.luogu.com.cn/problem/AT_agc002_d)

> - **Question**
>   - 图里有 `n` 个点， `m` 条无向边，边的编号 `1 ~ m` ，没有边权，所有点都连通，一共有 `q` 条查询，查询的格式如下：查询 `x y z` : 从两个点 `x` 和 `y` 出发，希望经过的点数量等于 `z` ，每个点可以重复经过，但是重复经过只计算一次，经过边的最大编号，最小是多少。
>   - **Tips**
>     - `3 <= n、m、q <= 10^5`
>     - `3 <= z <= n`
...
> - **Question: Kruskal重构树 + 重构树上每个节点建立叶节点数量的信息 + 通过z个不同的点转化成二分答案**
