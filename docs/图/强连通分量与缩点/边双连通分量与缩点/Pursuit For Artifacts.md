...
## [E. Pursuit For Artifacts](https://codeforces.com/problemset/problem/652/E)

> - **Question**
>   - 给定一张无向图，一共 `n` 个点、 `m` 条边，保证所有点连通，每条边除了端点之外，还有一个属性，值为 `1` 表示该边上有商品，值为 `0` 表示该边上无商品，给定起点 `s` 和终点 `t` ，路途怎么走随意，但是沿途每条边只能经过一次，从 `s` 到 `t` 的路途中能遇到商品打印“YES“，否则打印“NO”。
>   - **Tips**
>     - `1 <= n、m <= 3 * 10^5`
