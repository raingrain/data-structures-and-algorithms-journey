...
> - **Question**
>   - 给定一张无向图，一共 `n` 个点、 `m` 条边，所有点保证连通，图中可能有重边，一共有 `q` 条查询，每条查询格式 `s K1 K2 .. Ks` ，含义如下，当前查询涉及 `s` 个点，给出 `s` 个点的编号，你只能删掉一个另外的点，删除该点后，只要这 `s` 个点中存在任意两点不连通，就算成功，对每条查询，打印你能选择的点的个数。
>   - **Tips**
>     - `2 <= n <= 10^5`
>     - `1 <= m <= 2 * 10^5`
>     - `1 <= q <= 10^5`
