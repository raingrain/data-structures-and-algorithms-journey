...
## [E. Tourists](https://codeforces.com/problemset/problem/487/E)

> - **Question**
>   - 一共 `n` 个城市、 `m` 条双向道路，所有城市都连通，商品只有一种，每个城市给定商品报价，接下来有 `q`条操作，操作类型有两种，操作 `C x y` 城市 `x` 的商品报价改成 `y` ，操作 `A x y` 从 `x` 到 `y` 可自由选路，但不能有重复城市，打印能遇到的最低报价。
>   - **Tips**
>     - `1 <= n, m, q <= 10^5`
>     - `1 <= 商品报价 <= 10^9`
