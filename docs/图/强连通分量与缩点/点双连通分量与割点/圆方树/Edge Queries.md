...
## [F. Edge Queries](https://codeforces.com/problemset/problem/1763/F)

> - **Question**
>   - 给定一张无向图，一共 `n` 个点、 `m` 条边，所有点保证连通，一共 `q` 条查询，格式 `x y`，含义如下，所有从 `x` 到 `y` 的简单路径上出现的边中，统计有多少条边满足，如果移除该边， `x` 和 `y` 仍然可以互相到达，打印这个数量。
>   - **Tips**
>     - `1 <= n, m, q <= 2 * 10^5`
