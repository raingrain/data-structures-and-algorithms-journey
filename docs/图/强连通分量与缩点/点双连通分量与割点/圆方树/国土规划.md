...
> - **Question**
>   - 一张无向图有 `n` 个点、 `m` 条边，所有点连通，可能有重边，但是无自环，初始时所有点的状态都是 `0` ，一共 `q` 次操作，每次操作只有一个参数 `x` ，含义如下，点 `x` 的状态翻转， `0` 变 `1` ， `1` 变 `0` ，然后考虑如下问题，如果某个状态为 `0` 的点删除后，所有状态为1的点依然连通，这样的点叫无所谓点，每次操作后都打印无所谓点的数量。
>   - **Tips**
>     - `2 <= n <= 10^5`
>     - `1 <= m、q <= 2 * 10^5`
