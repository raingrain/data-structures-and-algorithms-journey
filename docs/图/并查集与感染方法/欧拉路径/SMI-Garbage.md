...
## [P3520 [POI 2011] SMI-Garbage](https://www.luogu.com.cn/problem/P3520)

> - **Question**
>   - 一共有 `n` 个点， `m` 条无向边，所有点不保证连通。
>   - 每条边的属性为 `u v s t` ： `u` 和 `v` 是端点， `s` 是初始状态， `t` 是最终状态。
>   - 状态的数值只有 `0` 和 `1` 两种，当一辆车通过一条无向边，那么状态会翻转。
