...
## [P3443 [POI 2006] LIS-The Postman](https://www.luogu.com.cn/problem/P3443)

> - **Question**
>   - 一共 `n` 个点， `m` 条有向边，不会有重边和自环。
>   - 邮递员一定要从 `1` 号点开始，最后回到 `1` 号点，每条边都要走 `1` 次。
>   - 邮递员依次走过的点的编号，就可以形成一个路径。
>   - 给定t个序列，每个序列先给定长度 `k` ，然后是 `k` 个节点的编号。
