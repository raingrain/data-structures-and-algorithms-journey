...
## [F - Coprime Solitaire](https://atcoder.jp/contests/abc210/tasks/abc210_f)

> - **Question**
>   - 一共 `n` 张卡片，每张卡片给定两个待选数字，选择其中一个数字作为卡片的值，确定每张卡片的值之后，如果任意两张卡片的值都是互质的，那么算作成功，存在成功的选值方案打印 `Yes` ，不存在打印 `No` 。
>   - **Tips**
>     - `1 <= n <= 3 * 10^4`
>     - `1 <= 待选数字 <= 2 * 10^6`
...
```java
// 利用欧拉筛，生成最小质因子表，进而分解数字
// 每个数字根据拥有的质数因子分配阵营，一个阵营内只能选一个数字
