...
## [#2155. 「POI2011 R1」同谋者 Conspiracy](https://loj.ac/p/2155)

> - **Question**
>   - 一共 `n` 个人，编号 `1 ~ n` ，分配每个人进入交流队或者保密队中的一个，交流队要求内部任意两人都认识，保密队要求内部任意两人都不认识，给定每人的认识列表，如果 `x` 的认识列表里有 `y` ，那么 `y` 的认识列表里有 `x` ，分配还要求交流队和保密队都不能为空，计算 `n` 个人分成两组的方法数，如果不存在任何方法，打印 `0` 。
>   - **Tips**
>     - `2 <= n <= 5000`
>     - `所有认识列表的总人数 <= n * (n - 1)`
