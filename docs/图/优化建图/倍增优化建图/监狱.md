...
## [P9520 [JOIST 2022] 监狱 / Jail](https://www.luogu.com.cn/problem/P9520)

> - **Question**
>   - 一共有 `n` 个房间，给定 `n - 1` 条无向边，所有房间组成一棵树，一共有 `m` 个囚犯，每个囚犯给出卧室房间号和工作室房间号，囚犯初始时都在卧室，每个囚犯的卧室和工作室一定不同，任何两个囚犯既不共用卧室，也不共用工作室，但是有可能某个房间，作为一个囚犯的卧室，同时作为另一个囚犯的工作室，你的任务是让所有囚犯从自己的卧室出发，只走最短路去自己的工作室，你可以随意下达指令，每条指令只能选择一个囚犯，沿一条边移动一步，任何时刻不能让任何房间里出现两个囚犯，打印你能否完成任务。
>   - **Tips**
>     - `1 <= n、m <= 1.2 * 10^5`

## Java

> - **Question: 策略证明 + 题目建模 + 倍增优化建图 + 拓扑排序判环**
