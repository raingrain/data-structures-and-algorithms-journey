...
## [P6348 [PA 2011] Journeys](https://www.luogu.com.cn/problem/P6348)

> - **Question**
>   - 一共 `n` 个点，给定起点 `p` ，一共 `m` 次操作，格式如下：操作 `a b c d` ： `a ~ b` 范围每个点与 `c ~ d` 范围每个点之间，都增加一条无向边，所有操作完成后，计算起点 `p` 到每个点经过的最少边数，题目保证整体连通。
>   - **Tips**
>     - `1 <= n <= 5 * 10^5`
>     - `1 <= m <= 10^5`
