...
> - **Question**
>   - 一共有 `n` 个炸弹，所有炸弹排成一条直线，给定每个炸弹的坐标 `xi` 、爆炸半径 `ri` ，炸弹 `A` 引爆时，如果炸弹 `B` 在其影响范围里，那么炸弹 `B` 也会引爆，进而引发一连串的爆炸，炸弹 `i` 如果作为初始引爆的炸弹，最终会引爆多少个炸弹记为 `query(i)` ，计算 `i = 1 2 ... n` 时， `i *query(i)` 的累加和，答案对 `1000000007` 取余。
>   - **Tips**
>     - `1 <= n <= 5* 10^5`
...
> - **Question: 线段树优化建图 + 强连通分量缩点 + DAG动态规划 + 建反图**
