...
> - **Question**
>   - 图中有 `n` 个点、 `n` 条无向边，每个点有点权，图中可能有多个连通块，保证每个连通块都是一棵基环树，图中任意相邻两点不能同时选择，计算能得到的最大点权累加和。
>   - **Tips**
>     - `1 <= n、点权 <= 10^6`
...
```java
// 选父时，子不选。不选父时，子可选可不选
// 找到环上两点x、y，以及它们之间的边skip，忽略该边，进行两次树型dp
// 第一次得到dp[x][0]，第二次得到dp[y][0]，答案 = max(dp[x][0], dp[y][0])
