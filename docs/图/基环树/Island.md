...
## [P4381 [IOI 2008] Island](https://www.luogu.com.cn/problem/P4381)

> - **Question**
>   - 图中有 `n` 个点、 `n` 条无向边，每条边有边权，图中可能有多个连通块，保证每个连通块都是一棵基环树，对每个连通块，计算点不重复的路径能得到的最大边权和，即基环树直径，图中每棵基环树的直径累加起来，打印这个结果。
>   - **Tips**
>     - `2 <= n <= 10^6`
>     - `1 <= 边权 <= 10^8`
...
// 掌握单调队列
// 与环无关的答案、与环有关的答案
// 指标为（当前树高 - 前缀和），单调队列维护环上信息，这种技巧很常见
