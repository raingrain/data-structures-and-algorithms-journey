...
## [F. Roads in the Kingdom](https://codeforces.com/problemset/problem/835/F)

> - **Question**
>   - 图中有 `n` 个点、 `n` 条无向边，每条边有边权，图是一棵基环树，可以任意删掉环上的一条边，让图变成树，希望让树的直径最小，第一个测试链接，计算树的直径最小值，第二个测试链接，等价于计算树的直径最小值 `/ 2` ，结果保留一位小数。
>   - **Tips**
>     - `1 <= n <= 2 * 10^5`
>     - `1 <= 边权 <= 10^9`
...
// 先掌握如何求树的直径
// 与环无关的答案、与环有关的答案
// 环上做动态规划，以节点u做分界线，跨分界线产生的答案、不跨分界线产生的答案
// 指标为（当前树高 - 前缀和），利用差分得到最优决策
