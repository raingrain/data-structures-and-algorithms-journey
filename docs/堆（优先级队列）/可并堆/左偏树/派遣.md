...
> - **Question**
>   - 一共有 `n` 个忍者，每个忍者有上级编号、工资、能力，三个属性。输入保证，任何忍者的上级编号小于这名忍者的编号， `1` 号忍者是整棵忍者树的头。你一共有 `m` 的预算，可以在忍者树上随意选一棵子树，然后在这棵子树上挑选忍者。你选择某棵子树之后，不一定要选子树头的忍者，只要不超过 `m` 的预算，可以随意选择子树上的忍者。最终收益等于雇佣人数乘以子树头忍者的能力，返回能取得的最大收益是多少。
>   - **Tips**
>     - `1 <= n <= 10^5`
>     - `1 <= m <= 10^9`
>     - `1 <= 每个忍者工资 <= m`
