...
## [P4971 断罪者](https://www.luogu.com.cn/problem/P4971)

> - **Question**
>   - 给定 `t, w, k` ，表示一共有 `t` 个人，死亡方式都为 `w` ，地狱阈值都为 `k` 。每个人都给定 `n` 和 `m` ，表示这人一生有 `n` 件错事，有 `m` 次领悟。】这个人的 `n` 件错事，给定对应的 `n` 个罪恶值，然后给定 `m` 次领悟，领悟类型如下：
>     - `2 a` ：第 `a` 件错事的罪恶值变成 `0` 。
>     - `3 a b` ：第 `a` 件错事所在的集合中，最大罪恶值的错事，罪恶值减少 `b` ，如果减少后罪恶值变成负数，认为这件错事的罪恶值变为 `0` ，如果集合中，两件错事都是最大的罪恶值，取编号较小的错事。
>     - `4 a b` ：第 `a` 件错事所在的集合与第 `b` 件错事所在的集合合并。
>   - 一个错事集合的罪恶值等于这个集合中的最大罪恶值，只取一个。
>   - 一个人的罪恶值等于这个人所有错事集合的罪恶值累加起来。
>   - 然后根据死亡方式 `w` ，对每个人的罪恶值做最后调整，然后打印这个人的下场：
>     - 如果 `w==1` ，不调整。
>     - 如果 `w==2` ，人的罪恶值 `-=` 错事集合的罪恶值中的最大值。
>     - 如果 `w==3` ，人的罪恶值 `+=` 错事集合的罪恶值中的最大值。
>   - 如果一个人的罪恶值等于 `0` ，打印"Gensokyo 0"。
>   - 如果一个人的罪恶值大于 `k` ，打印"Hell "，然后打印罪恶值。
>   - 如果一个人的罪恶值小于等于 `k` ，打印"Heaven "，然后打印罪恶值。
