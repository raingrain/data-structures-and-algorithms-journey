...
> - **Question**
>   - 一共有 `n` 个点，给定 `n - 1` 条无向边，所有点连成一棵树。一共有 `p` 个盘子，每个盘子格式 `a b c` ：盘子是点 `a` 到点 `b`的 路径，盘子权值为 `c` 。一共有 `q` 个水果，每个水果格式 `u v k` ：水果是点 `u` 到点 `v`的 路径， `k` 含义如下，如果一个盘子路径完全在一个水果路径的内部，那么该盘子可以接住该水果。那么对于每个水果，可能有很多盘子都可以将其接住，打印其中第 `k` 小的权值。
>   - **Tips**
>     - `1 <= n、p、q <= 4 * 10^4`
>     - `0 <= 盘子权值 <= 10^9`
>     - 内存可用空间 `500MB`
...
> - **普通整体二分思路的分析 + 路径包含的分析 + 发现二维树状数组不可用 + 树上倍增 + 扫描线 + 树状数组 + 修改和查询装在一起的整体二分**
