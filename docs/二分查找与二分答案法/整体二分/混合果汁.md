...
> - **Question**
>   - 一共有 `n` 种果汁，每种果汁给定，美味度 `d` 、每升价格 `p` 、添加上限 `l` ，制作混合果汁时每种果汁不能超过添加上限，其中美味度最低的果汁，决定混合果汁的美味度，一共有 `m` 个小朋友，给每位制作混合果汁时，钱数不超过 `money[i]` ，体积不少于 `least[i]` ，打印每个小朋友能得到的混合果汁最大美味度，如果无法满足，打印 `-1` 。
>   - **Tips**
>     - `1 <= n、m、d、p、l <= 10^5`
