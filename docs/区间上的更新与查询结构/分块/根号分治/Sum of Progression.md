...
## [F. Sum of Progression](https://codeforces.com/problemset/problem/1921/F)

> - **Question**
>   - 一共有 `t` 组测试，每组测试遵循同样的设定，给定一个长度为 `n` 的数组 `arr` ，接下来有 `q` 条查询，查询格式如下：查询 `s d k` ： `arr[s]` 作为第 `1` 项、 `arr[s + 1d]` 作为第 `2` 项、 `arr[s + 2d]` 作为第 `3` 项……每项的值乘以项的编号，一共 `k` 项都累加起来，打印累加和。
>   - **Tips**
>     - `1 <= n <= 10^5`
>     - `1 <= q <= 2 * 10^5`
