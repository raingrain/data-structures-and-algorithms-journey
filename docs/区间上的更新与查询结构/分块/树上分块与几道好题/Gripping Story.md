...
## [E. Gripping Story](https://codeforces.com/problemset/problem/198/E)

> - **Question**
>   - 磁块有五个属性值 `x, y, m, p, range` ，磁块在 `(x, y)` 位置、质量为 `m` 、磁力为 `p` 、吸引半径 `range` 。磁块A可以把磁块B吸到磁块A的位置，需要满足如下的条件：A与B的距离不大于A的吸引半径，并且B的质量不大于A的磁力。你有一个初始磁块，给定初始磁块的 `4` 个属性值（不给质量，因为没用），你永远在初始磁块的位置，接下来给定 `n` 个磁块各自的5个属性值，你可以用初始磁块，吸过来其中的磁块，吸过来的磁块可以被你随意使用，返回你最多能吸过来多少磁块。
>   - **Tips**
>     - `1 <= n <= 3 * 10^5`
>     - `-10^9 <= x、y <= 10^9`
>     - `1 <= m、p、range <= 10^9`
