...
## [C - 歴史の研究](https://atcoder.jp/contests/joisc2014/tasks/joisc2014_c)

> - **Question**
>   - 给定一个长度为 `n` 的数组 `arr` ，下面定义重要度的概念，如果一段范围上，数字 `x` 出现 `c` 次，那么这个数字的重要度为 `x * c` ，范围上的最大重要度，就是该范围上，每种数字的重要度，取最大值，一共有 `m` 条查询，格式 `l r` : 打印 `arr[l ~ r]` 范围上的最大重要度。
>   - **Tips**
>     - `1 <= n、m <= 10^5`
>     - `1 <= arr[i] <= 10^9`
