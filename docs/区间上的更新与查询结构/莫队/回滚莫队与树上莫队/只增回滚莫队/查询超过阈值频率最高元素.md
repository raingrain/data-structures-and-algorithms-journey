...
> - **Question**
>   - 给你一个长度为 `n` 的整数数组 `nums` 和一个查询数组 `queries` ，其中 `queries[i] = [li, ri, thresholdi]` 。返回一个整数数组 `ans` ，其中  `ans[i]` 等于子数组 `nums[li ~ ri]` 中出现 至少 `thresholdi` 次的元素，选择频率最高的元素（如果频率相同则选择最小的元素），如果不存在这样的元素则返回 `-1` 。
>   - **Tips**
