...
> - **Question**
>   - 给定一个长度为 `n` 的数组 `arr` ，给定一个非负整数 `k` ，下面给出 `k1` 二元组的定义：位置二元组 `(i, j)` ， `i` 和 `j` 必须是不同的，并且 `arr[i] ^ arr[j]` 的二进制状态里有 `k` 个 `1` ，当 `i != j` 时， `(i, j)` 和 `(j, i)` 认为是相同的二元组，一共有 `m` 条查询，格式为 `l r` : 打印 `arr[l ~ r]` 范围上，有多少 `k1` 二元组。
>   - **Tips**
>     - `1 <= n, m <= 10^5`
