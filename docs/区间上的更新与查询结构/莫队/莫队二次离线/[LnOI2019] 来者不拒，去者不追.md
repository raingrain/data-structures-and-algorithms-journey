...
> - **Question**
>   - 给定一个长度为 `n` 的数组 `arr` ，区间 `Abbi` 值的定义如下：如果 `arr[l ~ r]` 包含数字 `v` ，并且 `v` 是第 `k` 小，那么这个数字的 `Abbi` 值等于 `v * k` ，区间 `Abbi` 值等于区间内所有数字 `Abbi` 值的累加和，比如 `[1, 2, 2, 3]` 的 `Abbi` 值等于 `1 * 1 + 2 * 2 + 2 * 2 + 3 * 4 = 21` 。
