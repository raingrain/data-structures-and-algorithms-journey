...
> - **Question**
>   - 三个区间同时出现的数，一个一个删掉，直到无法再删，剩下数字的个数叫做剩余个数。
>   - `A = [1 2 2 3 3 3], B = [1 2 2 3 3 3], C = [1 1 2 3 3]` ，删除的过程为，一起删掉一个 `1` 、一起删掉一个 `2` 、一起删掉 `2` 个 `3` ，然后状况为 `A = [2 3], B = [2 3], C = [1]` ，剩余个数为 `5` 。
>   - 给定一个长度为 `n` 的数组 `arr` ，下来有 `m` 条查询，格式如下：查询 `l1 r1 l2 r2 l3 r3` ：给定了三个区间，打印剩余个数。
>   - **Tips**
>     - `1 <= n, m <= 10^5`
>     - `1 <= arr[i] <= 10^9`
