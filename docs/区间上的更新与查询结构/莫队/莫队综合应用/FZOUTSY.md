...
## [P5112 FZOUTSY](https://www.luogu.com.cn/problem/P5112)

> - **Question**
>   - 给定一个长度为 `n` 的字符串 `str` ，还有一个参数 `k` ，位置 `a` 开头和位置 `b` 开头的字符串，如果最长公共前缀的长度 `>= k` ，那么 `(a, b)` 构成 `lcp` 达标对，构成 `lcp` 达标对，必须是不同的位置，并且 `(a, b)` 和 `(b, a)` 只算一个达标对，不要重复统计，接下来有 `m` 条查询，格式为 `l r` ： `str[l ~ r]` 范围上，可以任选开头位置，打印 `lcp` 达标对的数量。
>   - **Tips**
>     - `1 <= n, k <= 3 * 10^6`
>     - `1 <= m <= 10^5`
>     - `1 <= n * n * m <= 10^15`
