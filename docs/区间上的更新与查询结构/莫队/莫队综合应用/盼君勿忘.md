...
## [P5072 [Ynoi Easy Round 2015] 盼君勿忘](https://www.luogu.com.cn/problem/P5072)

> - **Question**
>   - 一个序列中每种数字只保留一个，得到的累加和，叫做去重累加和，给定一个长度为 `n` 的数组 `arr` ，接下来是 `m` 条查询，查询格式如下：查询 `l r p` ： `arr[l ~ r]` 范围上，每个子序列的去重累加和，都累加起来 `% p` 的结果打印。
>   - **Tips**
>     - `1 <= n, m, arr[i] <= 10^5`
>     - `1 <= p <= 10^9`
