...
## [B. Friends](https://codeforces.com/problemset/problem/241/B)

> - **Question**
>   - 给定一个长度为 `n` 的数组 `arr` ，下标 `1 ~ n` ，你可以随意选两个不同位置的数字进行异或，得到两两异或值，顺序不同的话，算做一个两两异或值，那么，两两异或值，就有第1大、第2大，返回前 `k` 大两两异或值的累加和，答案对 `1000000007` 取模。
>   - **Tips**
>     - `1 <= n <= 5 *10^4`
>     - `0 <= k <= n* (n-1) / 2`
>     - `0 <= arr[i] <= 10^9`
