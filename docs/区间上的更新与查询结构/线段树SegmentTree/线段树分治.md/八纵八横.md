...
## [P3733 [HAOI2017] 八纵八横](https://www.luogu.com.cn/problem/P3733)

> - **Question**
>   - 一共有 `n` 个点，给定 `m` 条边，每条边的边权，用 `01` 字符串表达，初始时就保证图连通，初始的 `m` 条边永不删除，接下来有 `q` 条操作，每种操作是如下三种类型中的一种：
...
> - **Question: 时间轴线段树 + 环的路径异或和加入线性基 + 经典带权并查集 + 线性基的撤销 + 位图**
