...
>     - 操作 `1 x y` ：集合 `s` 中增加价值 `x` 、重量 `y` 的商品，商品编号自增得到。
>     - 操作 `2 x` ：集合 `s` 中删除编号为x的商品，删除时保证 `x` 号商品存在。
>     - 操作 `3` ：查询当前的 `f(s)` 。
>   - 定义 `a(s, m) = 集合s中挑选商品总重量<=m能获得的最大价值` 。
>   - 给定正数 `k, BAS, MOD` ，定义 `f(s) = ∑(m = 1...k) (a(s, m) *BAS的m-1次方 % MOD)` 。
>   - **Tips**
>     - `1 <= n <= 5 * 10^3`
>     - `1 <= q <= 3 * 10^4`
>     - `1 <= k、每件商品重量 <= 10^3`
