...
> - **Question**
>   - 数字范围 `1 ~ n` ，给定每种数字的个数，建立编号为 `1` 的可重集合，以后新集合的编号自增即可，接下来有 `m` 条操作，每条操作是如下五种类型中的一种：
>     - 操作 `0 x y z` ： `x` 号集合中，取出 `[y, z]` 范围上的所有数字，生成新的集合。
>     - 操作 `1 x y` ： `x` 号集合与 `y` 号集合合并，以后 `y` 编号的集合不会使用了。
>     - 操作 `2 x y z` ： `x` 号集合中，加入 `y` 个数字 `z` 。
>     - 操作 `3 x y z` ： `x` 号集合中，查询 `[y, z]` 范围上的数字个数。
>     - 操作 `4 x y` ： `x` 号集合中，查询第 `y` 小的数字，不存在打印 `-1` 。
>   - **Tips**
>     - `1 <= 所有数据 <= 2 * 10^5`
