...
> - **Question**
>   - 一共有 `n` 个节点，给定 `n - 1` 条边，所有节点组成一棵树，规定 `1` 号节点是树头，给定 `m` 个点对，每个点对 `(x, y)` ， `x` 是 `y` 的祖先节点，路径由从上到下的边组成，树上的每条边都要涂上白色或者黑色，完全由你决定，但是请保证每个点对的路径中，至少有一条黑色的边存在，打印给树涂色的方法数，答案对 `998244353` 取模。
>   - **Tips**
>     - `1 <= n、m <= 5 * 10^5`
