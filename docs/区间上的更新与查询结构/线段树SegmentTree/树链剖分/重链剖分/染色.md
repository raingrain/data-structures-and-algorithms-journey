...
> - **Question**
>   - 一共有 `n` 个节点，给定 `n - 1` 条边，节点连成一棵树，每个节点给定初始颜色值，连续相同颜色被认为是一段，变化了就认为是另一段，比如 `112221` 有三个颜色段，分别为 `11, 222, 1` ，一共有 `m` 条操作，每种操作是如下 `2` 种类型中的一种：
...
> - **Question: 重链剖分结合线段树 + 线段树的信息设计 + 多条重链信息的拼接**
