...
> - **Question**
>   - 一共有 `n` 个软件，编号 `0 ~ n - 1` ， `0` 号软件不依赖任何软件，其他每个软件都仅依赖一个软件，依赖关系由数组形式给出，题目保证不会出现循环依赖，一开始所有软件都是没有安装的，如果 `a` 依赖 `b` ，那么安装 `a` 需要安装 `b` ，同时卸载 `b` 需要卸载 `a` ，一共有 `m` 条操作，每种操作是如下 `2` 种类型中的一种：
>     - 操作 `install x` ：安装 `x` ，如果x已经安装打印 `0` ，否则打印有多少个软件的状态需要改变。
...
> - **Question: 重链剖分结合线段树 + 利用整棵线段树的信息变化**
