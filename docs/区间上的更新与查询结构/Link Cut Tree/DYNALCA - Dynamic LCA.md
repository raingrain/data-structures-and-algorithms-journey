...
> - **Question**
>   - 一共 `n` 个点，初始时每个点各自是一棵单点有根树，一共 `m` 条操作，操作类型如下：
>     - 操作 `link x y` : 让点 `x` 成为点 `y` 的儿子，保证 `x` 是所在树的根，并且 `x` 和 `y` 不连通，
>     - 操作 `cut x` : 删除点 `x` 和它父亲节点之间的直接边，保证 `x` 不是根，
>     - 操作 `lca x y`  : 查询点 `x` 和点 `y` 的最低公共祖先，保证 `x` 和 `y` 在同一棵树中。
>   - **Tips**
>     - `1 <= n、m <= 10^5`
