...
> - **Question**
>   - 一共 `n` 个点，初始时没有任何边，任意时刻图都是森林，一共 `m` 条操作，类型如下：
>     - 操作 `Connect x y` ：点 `x` 和点 `y` 之间增加直接边，保证原来不存在这条直接边，
>     - 操作 `Destroy x y` ：删除点 `x` 和点 `y` 之间的直接边，保证原来存在这条直接边，
>     - 操作 `Query x y` ：查询点 `x` 和点 `y` 之间是否连通，打印 `Yes` 或 `No` 。
>   - **Tips**
>     - `1 <= n <= 10^4`
>     - `1 <= m <= 2 * 10^5`
