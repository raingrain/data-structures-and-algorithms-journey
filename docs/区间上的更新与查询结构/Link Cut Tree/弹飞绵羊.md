...
> - **Question**
>   - 一共 `n` 个装置，编号 `0 ~ n - 1` ，从左到右摆成一条直线，每个装置给定弹力 `f` ，比如 `3` 号装置弹力为 `5` ，则从 `3` 号装置可以向右跳到 `8` 号装置，如果跳到的位置 `>= n` ，也就是不存在这个装置，则认为被弹飞，一共 `m` 条操作，操作类型如下：
>     - 操作 `1 x`   : 查询从 `x` 号装置出发，跳几次会弹飞，
>     - 操作 `2 x y` : `x` 号装置的弹力修改成 `y` 。
>   - **Tips**
>     - `1 <= n <= 2 * 10^5`
>     - `1 <= m <= 10^5`
