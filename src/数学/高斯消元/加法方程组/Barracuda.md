...
## [P5027 Barracuda](https://www.luogu.com.cn/problem/P5027)

> - ***Question***
>   - 一共有 `n` 个物品，编号 `1 ~ n` ，定义合法方案如下：
>     - 每个物品的重量都是确定的。
>     - 每个物品的重量一定都是正整数。
>     - 最重的物品有且仅有 `1` 个。
>   - 每次称重格式类似 `3 2 5 6 10` ，代表本次称重涉 `3` 个物品，编号为 `2, 5, 6` ，总重量 `10` 。
>   - 一共有 `n + 1` 条称重数据，称重数据整体有效的条件为：
>     - 错误的称重数据有且仅有 `1` 条，只有排除这条错误称重，才能求出一种合法方案。
>     - 如果称重数据有效，打印最重三角形的编号。
>     - 如果称重数据无效，打印 `illegal` 。
>   - ***tips:***
>     - `1 <= m <= n <= 100`
