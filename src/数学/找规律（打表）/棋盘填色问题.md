...
> - ***Question***
>   - `N * M` 的棋盘，每种颜色的格子数必须相同的，相邻格子染的颜色必须不同，所有格子必须染色，返回至少多少种颜色可以完成任务。
...
class Solution {
    ...
    public static int minColors(int N, int M) {
        ...
        for (int i = 2; i < N * M; i++) {
            ...
            // 下面这一句可知，需要的最少颜色数i，一定是N*M的某个因子，因为要保证每种颜色的格子数必须相同的
    ...
    public static boolean can(int[][] matrix, int N, int M, int pNum) {
        ...
        return recursion(matrix, N, M, pNum, 0, 0, rest);
    }
    
    public static boolean recursion(int[][] matrix, int N, int M, int pNum, int row, int col, List<Integer> rest) {
        if (row == N) {
            return true;
        }
        if (col == M) {
            return recursion(matrix, N, M, pNum, row + 1, 0, rest);
        }
        ...
        for (int color = 1; color <= pNum; color++) {
            if (color != left && color != up && rest.get(color) > 0) {
                ...
                if (recursion(matrix, N, M, pNum, row, col + 1, rest)) {
                    return true;
                }
