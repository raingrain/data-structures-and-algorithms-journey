...
> - ***Question***
>   - 一共有 `n` 个房间，编号 `1 ~ n` ，一开始都是空房
>   - 实现如下两种操作，会一共调用 `m` 次
>     - 操作 `1 x` ：找到至少有连续 `x` 个空房间的区域，返回最左编号，如果有多个满足条件的区域，返回其中最左区域的最左编号，如果找不到打印 `0` ，并且不办理入住，如果找到了打印最左编号，并且从最左编号开始办理 `x` 个人的入住。
>     - 操作 `2 x y` ：从 `x` 号房间开始往下数 `y` 个房间，一律清空。
>   - 操作 `1` 有打印操作，操作 `2` 没有。
>   - ***tips:***
>     - `1 <= n, m <= 50000`
