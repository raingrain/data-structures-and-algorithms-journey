...
> - **Question**
>   - **输入描述**
>     - 给定一个长度为 `n` 的数组 `arr` ，内部只有 `01` 两种值，下标从 `0` 开始。
>     - 对于这个序列有五种变换操作和询问操作：
>       - 操作 `0 l r` ：把 `l ~ r` 范围上所有数字全改成 `0` 。
>       - 操作 `1 l r` ：把 `l ~ r` 范围上所有数字全改成 `1` 。
>       - 操作 `2 l r` ：把 `l ~ r` 范围上所有数字全取反。
>       - 操作 `3 l r` ：询问 `l ~ r` 范围上有多少个 `1` 。
>       - 操作 `4 l r` ：询问 `l ~ r` 范围上连续 `1` 的最长子串长度。
>   - **输出描述**
