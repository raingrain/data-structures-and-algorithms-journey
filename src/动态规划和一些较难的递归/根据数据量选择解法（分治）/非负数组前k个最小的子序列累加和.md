...
> - ***Question***
>   - 给定一个数组 `nums` ，含有 `n` 个数字，都是非负数，给定一个正数 `k` ，返回所有子序列中累加和最小的前 `k` 个累加和，子序列是包含空集的。
>   - ***tips:***
>     - `1 <= n <= 10^5`
>     - `1 <= nums[i] <= 10^6`
>     - `1 <= k <= 10^5`
...
class Solution {
    ...
    public static int[] topKSum1(int[] nums, int k) {
        ...
        allSubsequences.sort(Comparator.naturalOrder());
        int[] ans = new int[k];
        for (int i = 0; i < k; i++) {
    ...
    // {1 3 2 4 5 6}
    // 小跟堆存储累加和，每次弹出累加和，把该子序列的下一个位置单独拉入，以及拉入与下一个位置构成的新序列的累加和
