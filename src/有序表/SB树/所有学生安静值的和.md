...
> - **Question**
>   - 给定一个非负数组 `arr` ，学生依次坐在 `0 ~ N - 1` 位置，每个值表示学生的安静值。
>   - 如果在 `i` 位置安置插班生，那么 `i` 位置的安静值变成 `0` ，同时任何同学都会被影响到而减少安静值，同学安静值减少的量等于数组长度减去这个同学到插班生的距离，但是减到 `0` 以下的话，当做 `0` 处理。
>   - 返回一个和 `arr` 等长的 `ans` 数组， `ans[i]` 表示如果把插班生安排在 `i` 位置，所有学生的安静值的和。
>   - **Tips**
>     - `arr.length <= 10^5`
>     - `arr[i] <= 2 * 10^5`
...
> - **有序表改写**
>   - 太恶心。
>   - 当前安静值加当前位置减去数组长度得到的指标表示插班生大于这个位置时我的安静值本来要变成负数但只能变成 `0` 。
