...
> - ***Question***
>   - 给定一个数组 `arr` ，表示从早到晚，依次会出现的导弹的高度。
>   - 大炮打导弹的时候，如果一旦大炮定了某个高度去打，那么这个大炮每次打完当前炮弹后的高度都必须等于当前炮弹高度减一。
>   - 如果只有一个大炮，返回最多能拦截多少导弹。如果所有的导弹都必须拦截，返回最少的大炮数量。

## Java

> - ***贪心 + 有序表***
>   - 第一题求炮弹高度数组中最长递减子序列的长度。
>   - 第二题的贪心策略为：如果已有的大炮不能拦截当前导弹，新开一门炮，如果能打，给大炮能打的高度与导弹高度差值最小的大。
