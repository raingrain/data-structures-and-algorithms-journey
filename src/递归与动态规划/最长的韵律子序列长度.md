...
>   - 比如 `1 1 3 3` 就属于 `AABB` 型的韵律， `6 6 6 6` 就属于 `AAAA` 型的韵律等等。
>   - 一个数组 `arr` ，当然可以生成很多的子序列，如果某个子序列一直以韵律的方式连接起来，我们称这样的子序列为韵律子序列（长度一定是 `4` 的倍数）。
>   - 给定一个数组 `arr` ，返回最长的韵律子序列长度。
>   - ***tips:***
>     - `0 <= arr.length <= 4000`
>     - `0 <= arr[i] <= 10^9`
