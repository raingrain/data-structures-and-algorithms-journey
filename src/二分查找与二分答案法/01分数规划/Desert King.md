...
## [Desert King](http://poj.org/problem?id=2728)

> - ***Question***
>   - 一共有 `n` 个村庄，每个村庄由 `(x, y, z)` 表示，其中 `(x, y)` 代表村庄在二维地图中的位置， `z` 代表其海拔高度，任意两个村庄之间的距离就是二维地图中的欧式距离，任意两个村庄之间的修路花费就是海拔差值的绝对值，现在想把所有村庄联通起来，希望修路的条数尽量少，同时希望让 `总花费 / 总距离`，这个比值尽量小，返回最小的比值是多少，结果保留小数点后 `3` 位其余部分舍弃。
>   - ***tips:***
>     - `2 <= n <= 10^3`
>     - `0 <= x、y <= 10^4`
>     - `0 <= z <= 10^7`
